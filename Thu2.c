...
#define MAX 4096
#define DEBUG 1
#define PRINTF_CASE(i)  printf("this is case %d\n", i);
#define SET_ZERO(i) i='\0';
#define ERROR printf("The input format: string1 string2 [parameter]"); 
...
int main() {
    ...
    while (ptr = getword(ptr, str)) {
        if (i <= 3) {
            if (i == 1) {
                ...
                SET_ZERO( targetstr[strlen(str)] )
            } else if (i == 2) {
                strncpy(replacestr, str, strlen(str));
                SET_ZERO( replacestr[strlen(str)] )
            } else {
                strncpy(parameter, str, strlen(str));
                SET_ZERO( parameter[strlen(str)] )
            }
        }
        i++;
    }
    ...
    #if DEBUG
        if (i == 3) PRINTF_CASE(0)
        else if (i == 4 && !strcmp(parameter, "-i")) PRINTF_CASE(1)
    #endif
